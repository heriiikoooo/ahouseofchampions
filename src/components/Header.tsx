...
const Header = () => {
  ...
  const navItems = [
    ...
    {
      name: "Next Steps",
      submenu: ["Salvation", "Water Baptism", "Small Groups", "Outreaches"],
    },
    {
      name: "Events",
      submenu: null,
        ...
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          <ul className="hidden md:flex gap-2 lg:gap-10 items-center">
            {navItems.map((item, index) => (
              ...
              >
                {/* Main Link - either a button for dropdowns or a Link for regular items */}
                {item.submenu ? (
                  <button
                    className="
                                          ...
                                          hover:after:scale-x-100 bg-transparent border-none cursor-pointer
                                      "
                  >
                    {item.name}
                  </button>
                  ...
                  <Link
                    ...
                    className="
                                          relative inline-flex items-center font-medium text-white text-sm lg:text-base
                                          px-3 py-2 pb-1 transition-all duration-500
                ...
                <li key={item.name}>
                  {item.submenu ? (
                    ...
                    <Link
                      to={routeMap[item.name] || "/"}
                      onClick={() => setMobileMenuOpen(false)}
                      className="block w-full text-left px-3 py-2 rounded-md text-white hover:bg-white/40 transition font-medium"
                    >
                      {item.name}
                    </Link>
                  )}
